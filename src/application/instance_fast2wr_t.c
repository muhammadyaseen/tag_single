...
int testapprun_tf(instance_data_t *inst, int message)
{
    ...
    switch (inst->testAppState)
    {
        case TA_INIT :
            ...
            switch (inst->mode)
            {
                case TAG:
                {
                	...
                	inst->encryption_data.key_index = 0;
        ...
        //this is the state for the tag which is called before the next ranging exchange starts
        //i.e. before the sending of next Poll or blink.

        //Notes : before sending poll, we need to look for RANGEINIT from 2nd Achor.
        ...
        case TA_TXBLINK_WAIT_SEND :
            {
                ...
                //ychange
                //dwt_setrxtimeout(0xFFFF);		//set to MAX timeout and try, ( 1.012e-6 sec x 2^16) = 66 ms
...
//                memcpy(&(inst->encryption_data.key), "1234", POLL_KEY_LEN);
                memcpy(&(inst->msg_f.messageData[PCODE]), &(inst->encryption_data.key), POLL_KEY_LEN);

    			inst->psduLength = TAG_POLL_F_MSG_LEN + POLL_KEY_LEN + FRAME_CRTL_AND_ADDRESS_S + FRAME_CRC + EXTRA_LENGTH;
           ...
           //printf("TA_TX_WAIT_CONF") ;
            {
                ...
                //tag will do immediate receive when waiting for report (as anchor sends it without delay)
                ...
                if(inst->previousState == TA_TXFINAL_WAIT_SEND)
                {
                    ...
                    //TODO:   need to change nextState here.

                    // after sending FINAL msg , tag should look for RANGEINIT msg from other Anchors rather than sending POLL to this same
                    // anchor.
                ...
                else
                {
                    ...
                    if(inst->previousState == TA_TXPOLL_WAIT_SEND)
					{
						...
						dwt_writetxdata((FRAME_CRTL_AND_ADDRESS_S + 2 + FRAME_CRC), (uint8 *)  &inst->msg_f, FINAL_MSG_OFFSET) ;   // write the final frame data
                ...
                case DWT_SIG_RX_OKAY :
                {
                    ...
                    else
                    {
                        ...
                        switch(fcode)
                        {
                            ...
                            {
//                            	uint8 code;
//								code = messageData[PCODE];
//                           		if(code != RESP_PASS_CODE)
//                           		{
//                           			uint8 dataseq[20];
//                           	        int j = 1000000;
//                           	        uint8 command[10];
//
//                           	        while(j--);
//                           	        command[0] = 0x1 ;  //clear screen
//                           	        writetoLCD( 1, 0,  &command);
//                           	        command[0] = 0x2 ;  //return cursor home
//                           	        writetoLCD( 1, 0,  &command);
//
//                                    sprintf((char*)&dataseq[0], "%llX", code);
//
//                           		    writetoLCD( 20, 1, (uint8 *)&dataseq);
//                           			while(1)
//                           				led_on(LED_ALL);
//                           		}
								...
								else
								{
									...
									diffRmP_non_encr = (uint32)dw_event->timeStamp32l - (uint32)inst->tagPollTxTime32l;
									rtd.diffRmP = encrypt_rc4(&(inst->encryption_data.key) , diffRmP_non_encr) ;
                            	...
                            	//ychange
                                                    	...
                                                    	writetoLCD( 1, 0,  &command);
                                                    	memset(dataseq, ' ', 40);
                                                    	memcpy(dataseq, (const uint8 *) "      rspons   ", 15);
                ...
                case DWT_SIG_RX_TIMEOUT :
                    ...
                    //ychange
                       ...
                       writetoLCD( 1, 0,  &command);
                       memset(dataseq, ' ', 40);
                       memcpy(dataseq, (const uint8 *) "     timeout   ", 15);
                       writetoLCD( 15, 1, dataseq); //send some data*/
                    //--
